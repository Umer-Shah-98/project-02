...
{savingsCard.map((element,index)=>(
    ...
    <div key={index} className={`flex-col justify-between m-1 ${classNames.join(" ")} rounded-md`}>
        ...
        <div className="info flex justify-around m-1 my-1">
            ...
            <div className="title-amount flex flex-col mt-3 ml-2 mr-3 justify-center items-center">
                ...
                <div className="amount"><span className='font-bold text-xs'>{currencyFormatter.format(amount)}</span>/<span className={`font-bold text-xs ${textColor}`}>{currencyFormatter.format(max)}</span></div>
            </div>
